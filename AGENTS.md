...
| Принцип, общий для всех языков (ошибки, конкурентность, безопасность, тесты, API, observability, ревью) | `shared/` |
...
- Коммиты — по правилам [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md): русский язык, без
  conventional-префиксов, один коммит — одна забота, `make validate` до коммита.
